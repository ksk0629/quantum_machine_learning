...
class TestYAngle:
    ...
    def test_build(self, data_dimension):
        ...
        3. itself, after performing decompose() method, contains only RY gates such that
        ...
        assert all(gate.operation.name == "ry" for gate in gates)
        #     3.1 the number of the gates is the same as data_dimension.
        assert len(gates) == data_dimension
        for gate in gates:
            #     3.2 they are parametrised.
            assert len(gate.operation.params) == 1  # == 1: since it's a RY gate

            applied_qubits.add(gate.qubits[0]._index)  # [0]: since it's a RY gate
        #     3.3 each gate is applied to each qubit.
        ...
        # 4-3. itself, after performing decompose() method, contains only RY gates such that
        decomposed_x_encoder = feature_map.decompose()
        gates = decomposed_x_encoder.data
        assert all(gate.operation.name == "ry" for gate in gates)
